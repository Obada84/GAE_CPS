...
def build_edges_grid(k_neighbors=8):
    ...
    return np.array(senders, dtype=np.int32), np.array(receivers, dtype=np.int32), positions / 27.0

def load_mnist_data(n_samples=10000, k_neighbors=8):
    print(f"Loading MNIST ({n_samples} samples, k={k_neighbors})...")
    t0 = time.time()
    ...
    print(f"  Loaded: {n_samples} samples | 784 nodes | {time.time()-t0:.1f}s")
...
class VGAEAutoEncoder(eqx.Module):
    ...
    def __init__(self, in_features, latent_dim, S1, S2, senders, receivers, n_nodes, key):
        ...
        self.dec_gcn_l0 = GCNLayer(8, in_features, adj_l0, keys[8], False)

    def encode(self, nodes):
        x = self.enc_gcn_l0(nodes); x = self.pool1.pool_nodes(x)
        x = self.enc_gcn_l1(x); x = self.pool2.pool_nodes(x)
        x = self.enc_gcn_l2(x); flat = x.reshape(-1)
        return self.enc_mlp_mu(flat), self.enc_mlp_log_var(flat)
    ...
    def encode_deterministic(self, nodes):
        mu, _ = self.encode(nodes)
        return mu

    def __call__(self, nodes, *, key):
        mu, log_var = self.encode(nodes)
...
def vgae_recon_loss(model, data, batch_axis):
    if isinstance(data, tuple):
        nodes_batch, sample_base_key = data
    else:
        nodes_batch = data
        sample_base_key = jr.PRNGKey(0)
    batch_size = nodes_batch.shape[0]
    keys = jr.split(sample_base_key, batch_size)

    def single_elbo(nodes, sample_key):
        recon, mu, log_var = model(nodes, key=sample_key)
        recon_loss = jnp.mean((recon[:, 2] - nodes[:, 2]) ** 2)
        kl = kl_divergence(mu, log_var)
        return recon_loss + KL_WEIGHT * kl

    losses = jax.vmap(single_elbo)(nodes_batch, keys)
...
def compute_recon_metrics(model, features):
    ...
    for start in range(0, features.shape[0], chunk_size):
        ...
        def single(nodes):
            mu = model.encode_deterministic(nodes)
            recon = model.decode(mu)
            return jnp.sqrt(jnp.mean((recon[:, 2] - nodes[:, 2]) ** 2))
        batch_rmses = jax.vmap(single)(chunk)
        rmses.extend(np.array(batch_rmses).tolist())
    return float(np.mean(rmses))

def main():
    print("MNIST VGAE Autoencoder")
    SEED = 42
    LATENT_DIM = 32
    STEPS = 10000
    BATCH = 64
    ...
    K_NEIGHBORS = 8
    k1, k2 = 200, 40
    MODEL_PATH = Path("mnist_vgae")
    t0 = time.time()
    print(f"  d={LATENT_DIM}, LR={LR}, BS={BATCH}, k={K_NEIGHBORS}, Pool={k1}/{k2}, KL_Weight={KL_WEIGHT}")

    all_features, labels, senders, receivers, positions = load_mnist_data(N_SAMPLES, K_NEIGHBORS)
    n_train = int(0.8 * N_SAMPLES)
    key = jr.PRNGKey(SEED); key, split_key = jr.split(key)
    perm = jr.permutation(split_key, N_SAMPLES)
    train_features = all_features[perm[:n_train]]; test_features = all_features[perm[n_train:]]
    ...
    for i in range(k1): S2[i, labels_2[i]] = 1.0
    S2 = jnp.array(S2)
  
    key = jr.PRNGKey(SEED); key, mk = jr.split(key)
    model = VGAEAutoEncoder(3, LATENT_DIM, S1, S2, senders, receivers, 784, mk)
  
    if MODEL_PATH.exists():
        model = eqx.tree_deserialise_leaves(MODEL_PATH, model)
    else:
        key, tk = jr.split(key)
        model, _ = klax.fit(model, 
                            train_features, 
                            batch_size=BATCH, 
                            batch_axis=0,
                            steps=STEPS, 
                            loss_fn=vgae_recon_loss, 
                            optimizer=optax.adam(LR),
                            history=klax.HistoryCallback(log_every=100),
                            batcher=batch_data_with_key, key=tk)
    ...
    all_results.append({'k1': k1, 'k2': k2, 'train_rmse': train_rmse, 'test_rmse': test_rmse})
